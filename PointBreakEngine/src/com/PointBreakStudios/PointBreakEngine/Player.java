...
//        this.summon(ypos, xpos, tag, ap, mas);
    ...
    public void checkInput(Input in){
        ...
        if(this.vely < -0.5F && !this.getTag().contains(new String("cursor")))
        {
            canjump = false;
            if(this.vely < -0.75F){
                this.vely = -0.8F;
        ...
        if(canjump && this.getTag().contains(new String("player1"))){
            
            this.vely = this.vely + (in.up() + in.down()) * 1.7F;
//            if(point2){this.vely = this.vely * -1;}
//            canjump = false;
        }
        if(canjump && this.getTag().contains(new String("player2"))){
...
//            this.setColor(new Color(0 + orange * 0.25F, 0 + orange * 0.5F, 0 + orange));
        ...
        if(this.getTag().contains(new String("player2"))){
...
//            this.setColor(new Color(0 + orange * 0.25F, 0 + orange * 0.5F, 0 + orange));
        ...
        if(this.getTag().contains(new String("cursor"))){
